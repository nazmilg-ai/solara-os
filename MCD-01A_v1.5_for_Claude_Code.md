...
Approved Draft Baseline means: this document is stable enough for
dependent Product Specifications (MCD-01B-xx) to be written against.
Further changes require a genuine architectural justification --- a gap
a future product actually exposes --- rather than editorial preference.
Product Specifications are now expected to conform to this document, not
to influence it.
...
not be duplicated across other layers.**
  ...
  **Layer**          **Examples**
  ...
  Configuration      Control side, bracket type, cassette colour, reverse
                     roll, motor selection

  Accessories        Remotes, extension brackets, tie-backs, hubs
  ------------------ ----------------------------------------------------
...
**Objective classification tests: Construction, Configuration, Accessory
(MCD-00A rule)**

These three tests replace subjective judgement with checkable questions,
mirroring the objective test for "Extended" in §7. Apply them in order:

-   1\. Construction/System test --- does the customer select it as the
    fundamental blind type, or does it change the product's construction
    or manufacturing route (headrail, brackets, assembly, tube/mechanism
    selection)? If yes, it is a Construction/System (or a Product, if
    the difference is severe enough --- see the Total Blackout Blinds
    note above). If no, continue to test 2.

-   2\. Configuration test --- is it a mandatory selection required to
    correctly install or operate the product, without changing the
    product's identity or construction? If yes, it is Configuration.
    Mandatory is the operative word: the product cannot be correctly
    installed or operated without this selection being made one way or
    another.

-   3\. Accessory test --- is it optional, addable or removable after
    the product has been configured, without the product losing its core
    identity or function? If yes, it is an Accessory. Optional is the
    operative word: the product still functions correctly without it.

Worked example (Roller Blinds --- the reference implementation,
MCD-01B-01): Cassette fails test 1 to pass (it changes the headrail,
bracket design, assembly, appearance, and sometimes the fabric deduction
and tube selection) --- Construction/System. Bracket Type, Control Side,
and Reverse Roll all fail test 1 (they don't change the underlying
construction) but pass test 2 (a bracket type, control side, and roll
direction must be chosen for correct installation) --- Configuration.
Remote, Smart Hub, Solar Panel, and Battery Charger all fail tests 1 and
2 (the blind operates correctly without any of them) but pass test 3 ---
Accessories.

> *Chain Length is deliberately not used as a Configuration example,
> despite superficially resembling Bracket Type or Control Side. It is
> not a selection at all --- it is a calculated output of the Child
> Safety Engine (MCD-01A §5.6), derived from drop. Where a genuine
> chain-related selection exists (e.g. choosing a chain safety device
> type), that selection is Configuration; the resulting chain length is
> not --- it is produced by the engine, not chosen by the advisor.
> Product Specifications should take care not to list calculated/derived
> values as if they were Configuration inputs.*
...
**Product Specifications shall implement each shared service through
product-specific configuration and shall not redefine or duplicate
shared behaviour.**
...
Full is expected to be uncommon for product-facing services and more
common for platform-administrative services, such as Document Generation
and Audit & Compliance. Most products will parameterise survey
questions, measurement rules, validation data, pricing inputs,
accessories, recommendations, or supplier-capability mappings to some
degree and will therefore normally be classified as Configured.

This expectation does not replace the objective assessment. Every Shared
Platform Service shall still be classified independently according to
the product's actual implementation.
...
a different product.
  ...
  **Shared Platform         **Status**   **Notes**
  ...
  Survey Engine             Configured   Standard survey workflow
                                         configured with Roller-specific
                                         questions, sequencing, and
                                         conditional branches

  Measurement Engine        Configured   Uses the shared Measurement
                                         Engine with Roller-specific
                                         measurement parameters such as
                                         reverse roll clearance, cassette
                                         clearance, and handle clearance

  Validation Engine         Configured   Uses the shared Validation Engine
                                         with Roller-specific
                                         compatibility rules, dimensional
                                         limits, and validation parameters

  Operation Engine          Configured   Supports Manual and Manual or
                                         Motorised (Advisor Selection)
                                         using existing shared operation
                                         modes

  Motor Engine              Configured   Enables compatible battery,
                                         mains, and solar motor options
                                         according to supplier capability

  Child Safety Engine       Configured   Applies shared BS EN 13120 logic
                                         with Roller-specific chain safety
                                         mappings

  Pricing Engine            Configured   Uses the shared Pricing Engine
                                         with Roller-specific pricing
                                         parameters including fabric
                                         bands, cassette options, and
                                         motor options

  Accessory Engine          Configured   Uses the shared Accessory Engine
                                         with Roller accessory groups and
                                         compatibility mappings

  Supplier Capability       Configured   Maps Roller-specific
  Engine                                 constructions, fabric
                                         collections, dimensional limits,
                                         tube systems, cassette and fascia
                                         availability, motor
                                         compatibility, control options,
                                         accessories, lead times, and
                                         availability status from the
                                         selected supplier's capability
                                         record

  Recommendation Engine     Configured   Uses the shared Recommendation
                                         Engine with Roller-specific
                                         recommendation rules and
                                         configuration data

  Document Generation       Full         Uses the shared Document
  Engine                                 Generation Engine without
                                         product-specific changes

  Audit & Compliance Engine Full         Uses the shared Audit &
                                         Compliance Engine without
                                         product-specific changes
  ------------------------- ------------ ---------------------------------

**Interpretation**

This worked example demonstrates the intended use of the Shared Service
Profile. The classifications shown are the result of applying the
objective assessment defined in §7 and shall not be treated as default
values for other products. Each Product Specification must independently
assess every Shared Platform Service against the objective test and
assign the appropriate status based on its own implementation.
...
Noted future consideration (not acted on now): the twelve Shared
Platform Services in §5 could eventually be classified by architectural
role --- e.g. Decision Engine (Recommendation), Calculation Engine
(Pricing, Child Safety), Validation Engine (Validation), Workflow Engine
(Survey), Repository/Mapping (Supplier Capability), Output Engine
(Document Generation). This distinction is not necessary today, but may
become useful if Solara OS grows to include Manufacturing OS, Customer
Portal, or Business Hub. Deliberately deferred rather than added now,
per the §10 governance rule: separate/reclassify only when it creates
value.
...
13\. Version History
  ...
  **Version**   **Summary**
  ...
  0.1           Initial per-product draft specifications (Roller, Vertical,
                Roman) with generic single-sentence sections.
  ...
  1.0 Draft     Full architectural consolidation: Architectural Layering
                principle, complete Product Taxonomy, twelve Shared Platform
                Services, six-status Shared Service Profile model with
                objective Extended test, document-splitting governance rule,
                and corrected Roller worked example. Supersedes all prior
                discussion.

  1.1 Draft     Corrected §7 and §11 after cross-checking against the first
                real product specification (MCD-01B-01 Roller Blinds v1.0):
                added an explicit \"Expected Usage Pattern\" note (Full is
                uncommon for product-facing services, common for
                platform-administrative ones), reclassified 6 of 12 services
                in the Roller worked example from Full/Extended to
                Configured after re-applying the objective test,
                strengthened the Supplier Capability Engine reasoning to
                name the specific fields Roller maps, and added an
                Interpretation note clarifying the worked example is not a
                set of default values for other products.

  1.2 Draft     Added §3 objective test for Construction/System vs.
                Accessories, promoted from resolving a real ambiguity found
                in Claude Code's formal review of MCD-01B-01 (whether
                Cassette is a Construction Variant or an Accessory). Mirrors
                the §7 objective test for Extended: replaces subjective
                judgement with a checkable question (can the concept be
                added/removed without changing the underlying
                construction?), so the same reasoning can be reapplied to
                future ambiguous cases without re-deriving it each time.

  1.3 Draft     Superseded the single §3 test with a complete three-test
                classification model (Construction, Configuration,
                Accessory), after applying it to a second ambiguity in
                MCD-01B-01 (Bracket Type) and recognising the single test
                could not distinguish Configuration from Accessory on its
                own. Added the mandatory-vs-optional discriminator between
                Configuration and Accessory. Added an explicit note that
                calculated/derived engine outputs (e.g. chain length,
                derived from drop by the Child Safety Engine) must not be
                listed as Configuration inputs, since Configuration is
                specifically about advisor selections, not engine
                calculations.

  1.4 Approved  Incorporated a second independent review (ChatGPT) of v1.3:
  Draft         strengthened §5's general statement from \"shall only
  Baseline      define\" to an explicit prohibition (\"shall implement\...
                and shall not redefine or duplicate shared behaviour\").
                Logged a deferred future consideration in §12 (classifying
                the 12 Shared Platform Services by architectural role ---
                Decision/Calculation/Validation/Workflow/Repository/Output
                --- relevant only if Solara OS grows beyond the Sales MVP
                scope) without acting on it now, per §10's own governance
                rule. Status formally changed from Draft to Approved Draft
                Baseline: stable enough for dependent Product Specifications
                to be written against; further changes require genuine
                architectural justification, not editorial preference.

  1.5 Approved  Final fix before genuine freeze: corrected a
  Draft         self-contradiction in §3's own layering table --- the
  Baseline      Accessories example list included bare \"brackets,\" which
                contradicted the Configuration row's own \"bracket type\"
                example and everything built afterward (the three-test
                model, the Roller worked example, MCD-01B-01 itself).
                Replaced with \"extension brackets\" (the correct
                Accessory-layer item) and added \"reverse roll\" to the
                Configuration examples for consistency with the Roller
                worked example. This is treated as a correctness fix, not
                editorial polish --- a frozen baseline should not contain a
                visible internal contradiction in its own foundational
                table. No other changes. MCD-01A is now frozen: future
                amendments require a genuine architectural gap exposed by a
                real product, per the Approved Draft Baseline status
                declared in v1.4.
  ------------- ------------------------------------------------------------
